...
public class CustomGravityProvider : LocomotionProvider, IGravityController
{
    [SerializeField]
    ...
    public bool gravityRotated = false;
    ...
    protected virtual bool TryProcessGravity(float time)
    {
        if (IsGravityBlocked())
        {
            //Debug.Log("Gravity is blocked.");
            ResetFallForce();
            return false;
        }

        //Debug.Log("Gravity is processing.");

        // Flip gravity direction dynamically
        Vector3 gravityForce = GetCurrentGravity();
        if (gravityInverted)
        {
        ...
        if (gravityRotated)
        {
            gravityForce = Quaternion.Euler(0, 0, -90 * rotationCount) * gravityForce;
        }
    ...
    {
        return m_UseLocalSpaceGravity ? -mediator.xrOrigin.Origin.transform.up * gravityMagnitude : gravityDirection;
    }
    ...
    {
        //Debug.Log($"IsGravityBlocked called. ForcedOff count: {m_GravityForcedOffProviders.Count}, ForcedOn count: {m_GravityForcedOnProviders.Count}");
    ...
    {
        Debug.Log($"Locking gravity from: {provider.GetType().Name}");
    ...
    {
        Debug.Log($"Unlocking gravity from: {provider.GetType().Name}");
